  ...
  return (
    <>
      ...
      >
        ...
        >
          Accessible typewriter with keyboard controls: Space (pause/resume), Escape (skip), R
          (reset)
        </div>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div style={{ fontSize: '12px', color: '#666', marginTop: '8px' }}>
          Segments: {metrics.totalSegments} | Visible: {metrics.visibleSegments} | Virtualized:{' '}
          {metrics.isVirtualized ? 'Yes' : 'No'} | Reduced Motion:{' '}
          {typewriter.getState().reducedMotion ? 'Yes' : 'No'}
        </div>
      )}
    </>
  );
